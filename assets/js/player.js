...
const playerExpGain = () => {
    ...
    if (typeof addPetExp === "function" && enemy && enemy.rewards && enemy.rewards.exp > 0) {
        var petShare = typeof PET_EXP_SHARE_FROM_PLAYER === "number" ? PET_EXP_SHARE_FROM_PLAYER : 0.27;
        addPetExp(Math.max(0, Math.floor(enemy.rewards.exp * petShare)));
    }
...
const playerLvlUp = () => {
    ...
    // Increase player bonus stats per level（原 3 倍速成长，现为 1 倍）
...
const playerLoadStats = () => {
    ...
    if (player.inCombat || playerDead) {
        ...
        if (playerInfoElement) {
            playerInfoElement.innerHTML = `<span class="combat-card__pname">${escapeHtml(player.name)}</span><span class="combat-card__prealm">${cultivationRealmLabel(player.lvl)}</span>`;
        }
        if (typeof refreshPetCombatHud === "function") refreshPetCombatHud();
    }

    // Header
    document.querySelector("#player-name").innerHTML = `<i class="fas fa-user"></i><span>${escapeHtml(player.name)}</span>`;
    var avEl = document.querySelector(".hud-bar__avatar");
    if (avEl) {
        if (player.avatar && typeof player.avatar === "string" && player.avatar.indexOf("data:") === 0) {
            avEl.classList.add("hud-bar__avatar--custom");
            avEl.style.backgroundImage = "url(" + JSON.stringify(player.avatar) + ")";
        } else {
            avEl.classList.remove("hud-bar__avatar--custom");
    ...
    var s = function (a, b, c) {
        c = c || 0;
        return (Number(a) + Number(b) + Number(c)).toFixed(2).replace(rx, "$1");
    };
    var setNote = "";
    var sbSum =
        (sb.hp || 0) +
    ...
    if (sbSum > 0) {
        setNote =
            '<p class="stat-card__note stat-card__note--set">身着遗器套装已叠加机缘（2/4/6 件同套）；下列含出战灵宠机缘（若有）。</p>';
    } else if (petSum > 0) {
        setNote = '<p class="stat-card__note stat-card__note--set">已含<strong>出战</strong>灵宠反哺机缘；与道体加算。</p>';
    }
    document.querySelector("#bonus-stats").innerHTML = `
    <h4 class="card__title">机缘加成</h4>
    <div class="stat-card__body">
    <p><i class="fas fa-heart"></i><span class="stat-card__lbl">气血</span><span class="stat-card__val">+${s(player.bonusStats.hp, sb.hp, petBk.hp)}%</span></p>
    <p><i class="ra ra-sword"></i><span class="stat-card__lbl">力道</span><span class="stat-card__val">+${s(player.bonusStats.atk, sb.atk, petBk.atk)}%</span></p>
    <p><i class="ra ra-round-shield"></i><span class="stat-card__lbl">护体</span><span class="stat-card__val">+${s(player.bonusStats.def, sb.def, petBk.def)}%</span></p>
    <p><i class="ra ra-plain-dagger"></i><span class="stat-card__lbl">身法</span><span class="stat-card__val">+${s(player.bonusStats.atkSpd, sb.atkSpd, petBk.atkSpd)}%</span></p>
    <p><i class="ra ra-dripping-blade"></i><span class="stat-card__lbl">吸血</span><span class="stat-card__val">+${s(player.bonusStats.vamp, sb.vamp, petBk.vamp)}%</span></p>
    <p><i class="ra ra-lightning-bolt"></i><span class="stat-card__lbl">会心</span><span class="stat-card__val">+${s(player.bonusStats.critRate, sb.critRate, petBk.critRate)}%</span></p>
    <p><i class="ra ra-focused-lightning"></i><span class="stat-card__lbl">暴伤</span><span class="stat-card__val">+${s(player.bonusStats.critDmg, sb.critDmg, petBk.critDmg)}%</span></p>
